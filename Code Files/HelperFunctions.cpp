...
bool isNumeric(string input) {
	...
	for (char character : input) {
		// If any character is not a digit, the number is not acceptable in this case
...
bool isValidKey(string key) { // Valid options: C, Db, D, Eb, E, F, F#, G, Ab, A, Bb, B
	...
	case 'D':
		if (key.length() == 1) return true;
		if (key.at(1) == 'b' && key.length() == 2) return true;
		break;
	case 'E':
		if (key.length() == 1) return true;
		if (key.at(1) == 'b' && key.length() == 2) return true;
	...
	case 'F':
		if (key.length() == 1) return true;
		if (key.at(1) == '#' && key.length() == 2) return true;
		break;
	case 'G':
		if (key.length() == 1) return true;
		break;
